...
object Day02 {
  ...
  object Password {
    private val pattern: Regex = "(\\d+)-(\\d+) (\\w): (\\w+)".r
    def apply(line: String): Try[Password] = Try {
      line match {
        case pattern(min, max, char, value) => Password(min.toInt, max.toInt, char.head, value)
  ...
  def parseInput(input: Seq[String]): Seq[Password] =
    input.flatMap(l => Password(l).toOption)
  ...
  def main(args: Array[String]): Unit = {
    val lines = Source.fromResource("day02/input.txt").getLines().toSeq
