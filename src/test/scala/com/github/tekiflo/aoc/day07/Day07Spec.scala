...
class Day07Spec extends AnyFlatSpec with Matchers {
  ...
  val bags: Day07.Bags = Day07.parseInput(input.split('\n').toSeq)
  ...
  it should "return the correct result for part 1 given the test input" in {
    bags.howManyCanContain(Day07.myBag) mustBe 4
  }

  it should "return the correct result for part 2 given the test input" in {
    bags.howManyBagsFor(Day07.myBag) mustBe 32
    ...
    val otherBags: Day07.Bags = Day07.parseInput(otherInput.split('\n').toSeq)
    otherBags.howManyBagsFor(Day07.myBag) mustBe 126
  }
}
